...
def plot_timeseries_geographic(timeseries_file, date_idx=None,
    ...
    im = ax.pcolormesh(lon, lat, displacement_cm,
    ...
    ax.plot(ref_lon, ref_lat, 'ks', markersize=10,
            markerfacecolor='black', markeredgecolor='white',
            markeredgewidth=2, label='Reference Point')
    ...
    # Title
    if title is None:
        ref_date = metadata.get('REF_DATE', dates[0]).decode() if isinstance(metadata.get('REF_DATE', dates[0]), bytes) else metadata.get('REF_DATE', dates[0])
...
def plot_velocity_geographic(velocity_file,
                             figsize=(12, 10), cmap='RdYlBu_r',
                             vmin=None, vmax=None,
    ...
    im = ax.pcolormesh(lon, lat, velocity_cm,
                       cmap=cmap, vmin=vmin, vmax=vmax,
                       shading='auto')
...
def interactive_timeseries_viewer(timeseries_file):
    ...
    im = ax.pcolormesh(lon, lat, timeseries_cm[0],
    ...
    ref_lon = lon[int(metadata['REF_Y']), int(metadata['REF_X'])]
    ref_lat = lat[int(metadata['REF_Y']), int(metadata['REF_X'])]
    ref_point = ax.plot(ref_lon, ref_lat, 'ks', markersize=10,
                       markerfacecolor='black', markeredgecolor='white',
                       markeredgewidth=2, label='Reference Point')[0]
    ...
    title = ax.set_title(f'N = 0, Time = {dates[0]}\nReference: N = 0, Time = {ref_date}',
    ...
    slider = Slider(ax_slider, 'Image', 0, len(dates)-1,
                   valinit=0, valstep=1)

    def update(val):
        idx = int(slider.val)
        im.set_array(timeseries_cm[idx].ravel())
        title.set_text(f'N = {idx}, Time = {dates[idx]}\nReference: N = 0, Time = {ref_date}')
...
Examples:
  ...
  %(prog)s timeseries.h5 --date-idx 44 --output displacement.png

  # Interactive timeseries viewer with slider
  %(prog)s timeseries.h5 --interactive

  # Plot velocity map
  %(prog)s velocity.h5 --output velocity.png

  # Custom color scale
  %(prog)s velocity.h5 --vmin -5 --vmax 5 --cmap jet
        """
    )

    parser.add_argument('input', help='Input HDF5 file (timeseries.h5 or velocity.h5)')
    parser.add_argument('--date-idx', type=int, default=None,
    ...
    if is_velocity:
        # Plot velocity
        if args.interactive:
            print("Warning: --interactive mode is only supported for timeseries files. Ignoring.")
        ...
        if args.interactive:
            if args.output:
                print("Warning: --output is not supported in interactive mode. Ignoring.")
